...
// =========================================================
// LEGACY BIT-BANG MODE + TCLOGO PWM AVERAGING + DEADBAND
// =========================================================
...
// TCLOGO PWM detection + averaging
bool legacyStreamingPWM = false;
uint32_t legacyWindowStartMs = 0;
uint32_t legacyCountTotal    = 0;
uint32_t legacyCountOn       = 0;
uint8_t  legacyLastNonZero   = 0;

// Deadband smoothing
uint8_t legacyPrevPWM = 0;
const uint8_t PWM_DEADBAND = 30;
...
void loopLegacy() {
  ...
  if (legacyWindowStartMs == 0) {
    legacyWindowStartMs = millis();
  }
  ...
  if (SerialBT.available() > 0) {
    ...
    if (peekByte == '#') {
      ...
      lastCommandTime = millis();
      legacyStreamingPWM = false;
      legacyWindowStartMs = 0;
      legacyCountTotal = 0;
      legacyCountOn    = 0;
      legacyLastNonZero = 0;
      legacyPrevPWM = 0;
    ...
    while (SerialBT.available() > 0) {
      ...
      if ((nowMs - legacyWindowStartMs) >= 80) {
        ...
        if (legacyStreamingPWM && legacyCountTotal > 0) {

          // Compute duty
          float duty = (float)legacyCountOn / (float)legacyCountTotal;
          uint8_t newPWM = (uint8_t)(duty * 255.0f);

          // Apply deadband smoothing
          if (abs((int)newPWM - (int)legacyPrevPWM) > PWM_DEADBAND) {
            legacyPrevPWM = newPWM;
          }

          // Apply PWM to all bits that were ON
          for (int i = 0; i < 6; i++) {
            if (legacyLastNonZero & (1 << i)) {
              ledcWrite(LEDC_CHANNELS[i], legacyPrevPWM);
            } else {
              ledcWrite(LEDC_CHANNELS[i], 0);
            }
          }
        ...
        // Reset window
        legacyWindowStartMs = nowMs;
        legacyCountTotal    = 0;
        legacyCountOn       = 0;
        legacyLastNonZero   = legacyOutputByte;

      } else {
        // Within window: for non-streaming, update immediately
        if (!legacyStreamingPWM) {
          for (int i = 0; i < 6; i++) {
            uint8_t val = (legacyOutputByte & (1 << i)) ? 255 : 0;
            ledcWrite(LEDC_CHANNELS[i], val);
          }
...
void setup() {
  ...
  // PWM outputs (Blockly + Legacy via LEDC)
  for (uint8_t i = 0; i < 6; i++) {
